...
int _HTTPClient::SQLMatchURLCallback(void* _reference, int _field_length, char** _field, char** _field_name) {
	_HTTPClient* _client = (_HTTPClient*)_reference;
...
_HTTPClient::_HTTPClient(HTTPEvent* _event, 
	...
	if(curl) {
		...
		curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, _HTTPClient::ReadBody);
		curl_easy_setopt(curl, CURLOPT_HEADERFUNCTION, ReadHeader);
		curl_easy_setopt(curl, CURLOPT_WRITEHEADER, this);
		curl_easy_setopt(curl, CURLOPT_WRITEDATA, this);
		
		curl_easy_setopt(curl, CURLOPT_NOPROGRESS, 0L);
		curl_easy_setopt(curl, CURLOPT_PROGRESSFUNCTION, _HTTPClient::Progress);
...
size_t _HTTPClient::ReadHeader(char* _stream, size_t _size, size_t _count, void* _pointer) {
	_HTTPClient* _client = (_HTTPClient*)_pointer;
...
size_t _HTTPClient::ReadBody(char* _stream, size_t _size, size_t _count, void* _pointer) {
	_HTTPClient* _client = (_HTTPClient*)_pointer;
	size_t _length = _size * _count;
...
int _HTTPClient::Progress(void* _client_pointer, 
						  double _download_total, 
						  double _download_now, 
						  double _upload_total, 
						  double _upload_now) {
	_HTTPClient* _client = (_HTTPClient*)_client_pointer;
...
bool _HTTPClient::UpdateDB() {
	...
	if(cache_type == HTTPResponse::CacheType_None) {
		...
		if(db_id.length()) {
			_query = "INSERT INTO request (url, Expires, Last_Modified) VALUES ('" + request.url + "', " + _expires + ", " + _last_modified + ")";
			_is_insert = true;
		} else {
...
bool _HTTPClient::PrevUpdateAndGetNeedDelete() {
	if(cache_type == HTTPResponse::CacheType_Expires) {
...
void _HTTPClient::MessageReciever(CURLcode _code) {
	if(header.is_open())
		header.close();
	if(body.is_open())
		body.close();
